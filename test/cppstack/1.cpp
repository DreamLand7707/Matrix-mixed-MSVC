...
int main(void)
{
    ...
    for (;;)
    {
        ...
        if (t1 == 0)
        {
            ...
            heap_init(block_heap, tx_heap);
            cout << "[TIME]:" << double(begin) / CLOCKS_PER_SEC << " seconds" << endl;
        }
        else if (t1 == 1)
        {
            ...
            while (1)
            {
                ...
                if (t2 == 10)
                {
                    ...
                    while (1)
                    {
                        ...
                        {
                            begint1 = clock();
                            t3 = search_admin_tx(tstr1, block_time, std::stol(tstr3), std::stol(tstr4), res_out, res_in);
                            begin += (clock() - begint1);
                        }
                        else
                        {
                        ...
                        }
                        {
                            ...
                            for (int i = 0; i < t1; i++)
                            {
                                if (res_out.empty() && !res_in.empty())
                                {
                                    ttr = res_out[t2++];
                                }
                                else if (!res_out.empty() && res_in.empty())
                                {
                                    ttr = res_out[t3++];
                                }
                                else if (t2 != res_out.size() && res_out[t2]->amout > res_in[t3]->amout)
                                {
                                    ttr = res_out[t2++];
                                }
        ...
        {
            t2 = 50;
            cout << "Please putin the time and num of items you want,the default value is 50" << endl;
            while (1)
            {
                getline(cin, tstr1);
                ...
                cout << "***************************************" << endl;
                for (int i = 0; i < t2 && i < admins.size(); i++)
                {
        ...
        else if (t1 == 4)
        {
            begint1 = clock();
            cout << "[HELP FILE]:" << endl
                 << "USE init to init block chain, but you can only do it once" << endl
                 << "USE balance to do sth. with account" << endl
                 << "\t then USE record to view records" << endl
                 << "\t or USE sum to view aggregate amount" << endl
                 << "USE rich man to view rich list" << endl
                 << "USE garph info to view sth. with graph" << endl
                 << "\t the USE inout" << endl
                 << "\t or USE circle" << endl
                 << "\t or USE from" << endl
                 << "USE add file to add a tx file" << endl;
            begin += (clock() - begint1);
            cout << "[TIME]:" << double(begin) / CLOCKS_PER_SEC << " seconds" << endl;
        }
        else if (t1 == 5)

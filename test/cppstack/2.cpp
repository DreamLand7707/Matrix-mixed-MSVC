...
{
    tx_heap.clear();
    for (auto &i : block_heap)
        for (auto &j : i.second.trans)
        {
            tx_heap[j.addr_in].as_in.push_back(&j);
            tx_heap[j.addr_out].as_out.push_back(&j);
        }
}
               ...
               std::map<std::string, double> &all_path)
{
    ...
    while (qu1.size())
    {
        ...
        qu1.pop_front();
        qu0.front().second--;
        auto k = all_path.find(t.first);
        if ((k == all_path.end()) || (k->second > (t.second->amout + qu0.front().first)))
        ...
        if (initd)
        {
            qu0.push_back({qu0.front().first + t.second->amout, t1});
        }
